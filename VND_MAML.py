...
def current_instance_features():
    """Return the loaded scheduling instance in the predictor's input format."""
    return {
        "N": N,
        "T": T,
        "B": B,
...
def run_once(seed: int, neighbor_order, time_limit=60, instance=None):
    """Run one VND experiment and return its runtime and objective value."""
    global NEIGHBOR_ORDER
    if instance is not None:
        configure_instance(instance)
    validated_order = tuple(int(item) for item in neighbor_order)
    if (
        not validated_order
        or len(set(validated_order)) != len(validated_order)
        or any(item not in range(1, 7) for item in validated_order)
    ):
        raise ValueError(
            "The neighborhood sequence must contain distinct IDs from 1 to 6."
        )
    NEIGHBOR_ORDER = list(validated_order)
    np.random.seed(seed)
    random.seed(seed)
...
if __name__ == "__main__":
    from Predict_VND import select_vnd_configuration

    instance = current_instance_features()
    print("Scheduling instance loaded from config.py")

    selection_start = time.perf_counter()
    selection = select_vnd_configuration(instance)
    selection_elapsed = time.perf_counter() - selection_start
    selected_order = selection["configuration"]
    print("\nSelected VND neighborhood sequence:", selected_order)
    print(f"Configuration-selection time: {selection_elapsed:.2f} s")

    run_times, objectives = [], []
    for run_id in range(maml_settings.NUMBER_OF_RUNS):
        t, obj = run_once(run_id + 1, selected_order)
        run_times.append(t)
