...
def _add_heuristic_columns(strategy):
    ...
    for key, value_list in best_Match.items():
        ...
        for tup in value_list:
            if 0 <= tup[0] - 1 < N:
                Vector[tup[0] - 1] = 1
        fingerprint = (tuple(sorted(Schedule)), key)
        protected_cols.add(fingerprint)
        Target = _calculate_target(Schedule, key)[0]
        _add_column(Schedule, Target, Vector, key)
    print("Heuristic Strategy Generation Solution:", best_Total)
...
def run_once():
    ...
    strategy        = random.choice(['used', 'cost', 'maxcols'])
    max_iter        = random.choice([40, 60, 80, 100])
    max_new_cols    = random.choice([5, 10, 15])

    initialize_model()
    column_generation_loop(max_iter=max_iter, tol=150, max_new_cols=max_new_cols)
    usage_list = remain_usage_based()
    Heuristic_solution(usage_list, strategy)
    # convert_to_integer()
    # master_model.optimize()
    ...
    print(f"[strategy={strategy}  max_iter={max_iter}  max_new_cols={max_new_cols}  cost={cost:.2f}  time={elapsed:.2f}s")
    return elapsed, cost
