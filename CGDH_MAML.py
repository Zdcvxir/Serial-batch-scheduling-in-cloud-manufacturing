...
def current_instance_features():
    """Return the current scheduling instance in the predictor's input format."""
    (
        current_N,
        current_T,
        current_B,
...
def run_once(
    ...
    """Run CGDH once with a reproducible random seed."""
    ...
    max_iter, max_new_cols, policy = configuration
...
if __name__ == "__main__":
    ...
    for run_id in range(maml_settings.NUMBER_OF_RUNS):
        t, obj = run_once(run_id + 1, selected_configuration)
        run_times.append(t)
